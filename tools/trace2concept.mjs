...
// rivers: black lines in the valleys — a shallow cut in the lowland, the gorge inside the canyon
...
const rivers = join(T.rivers || [], 30).filter(p => len(p) >= 80);
const riverValleys = rivers.map(p => { const mid = p[Math.floor(p.length / 2)]; const canyon = (T.canyon || []).some(poly => inPoly(mid, poly)); return canyon ? { pts: rnd(p), d: -140, r: 60 } : { pts: rnd(p), d: -16, r: 24 }; });
...
for (const p of crest.filter(p => len(p) >= 150)) {
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    // the range tapers over the last 280 px at each end, so the Crest comes down to the valley
    if (i) run += Math.hypot(p[i][0] - p[i - 1][0], p[i][1] - p[i - 1][1]);
    const endK = Math.min(1, Math.min(run, total - run) / 280);
    hs.push(Math.round(70 + (380 - 70) * endK));
    if (i % 6 === 3 && endK > 0.9) peaks.push({ x: Math.round(p[i][0] + nx * 150), z: Math.round(p[i][1] + ny * 150), h: 430 + ((i * 37) % 90), r: 90 });
  }
  ridges.push({ pts: rp, hs, h: 380, r: 170 });
}
...
const C = {
  ...
  regions: [
    ...
    ...(T.canyon || []).map(pts => ({ pts: rnd(pts), h: 130, edge: 90 })),
    ...(T.sea || []).map(pts => ({ pts: rnd(pts), d: -32, edge: 70 })),
    ...(T.lakes || []).map(pts => ({ pts: rnd(pts), d: -22, edge: 24 })),
  ],
